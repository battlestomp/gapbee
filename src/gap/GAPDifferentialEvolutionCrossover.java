 ...
 */
public class GAPDifferentialEvolutionCrossover extends Crossover {
	/**
  ...
  private static final List VALID_TYPES = Arrays.asList(IntSolutionType.class) ;
	 ...
	 * Constructor
	 */
	public GAPDifferentialEvolutionCrossover(HashMap<String, Object> parameters) {
		super(parameters) ;
		...
		if ((DE_Variant_.compareTo("rand/1/bin") == 0) || 
			...
			for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = parent[2].getDecisionVariables()[j].getValue()
							+ F_ * (parent[0].getDecisionVariables()[j].getValue() - parent[1].getDecisionVariables()[j].getValue()) ;
					...
					child.getDecisionVariables()[j].setValue((int)value);
				}
				else {
					double value ;
